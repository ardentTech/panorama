...
data class KPropConfig<T: Any>(
    ...
    companion object {
        ...
        private fun from(def: LexiconBlob, isNullable: Boolean = false, name: String): KPropConfig<String> {
            ...
            def.accept?.let { validators.add(KPropValidations.membership(name, it)) }
            def.maxSize?.let { validators.add(KPropValidations.lte(name.toByteArray().count(), it)) }
        ...
        private fun from(def: LexiconBytes, isNullable: Boolean = false, name: String): KPropConfig<String> {
            val validators = mutableListOf<String>()
            def.maxLength?.let { validators.add(KPropValidations.lte(name.count(), it)) }
            def.minLength?.let { validators.add(KPropValidations.gte(name.count(), it)) }
            return KPropConfig(String::class, null, null, isNullable, null, name, validators)
        }

        private fun from(def: LexiconInteger, isNullable: Boolean = false, name: String): KPropConfig<Int> {
            val validators = mutableListOf<String>()
            def.enum?.let { validators.add(KPropValidations.membership(name, it))}
            def.maximum?.let { validators.add(KPropValidations.lte(name, it)) }
            def.minimum?.let { validators.add(KPropValidations.gte(name, it)) }
